...
export class DashboardController {
    ...
    async dashboard(req: GaoRequest, res: GaoResponse) {
        ...
        if (!hasData && !skipOnboarding) {
            const progress = {
                products_added: (stats as any).totalProducts > 0 || false,
                contacts_added: stats.totalContacts > 0,
                deals_created: stats.totalDeals > 0,
                quotations_sent: (stats as any).totalQuotations > 0 || false,
            ...
            return res.html(renderPage({ title: 'Welcome', content, activePath: '/', user: user ? { name: user.name as string, role: user.role as string } : undefined }));
        }
