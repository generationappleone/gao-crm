...
async function main() {
    ...
    for (const stage of stages) {
        await driver.execute(`INSERT INTO deal_stages (id, name, slug, display_order, color, is_won, is_lost)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             ON CONFLICT (slug) DO NOTHING`, [stage.id, stage.name, stage.slug, stage.display_order, stage.color, stage.is_won, stage.is_lost]);
    }
    ...
    for (const deal of deals) {
        await driver.execute(`INSERT INTO deals (id, contact_id, company_id, owner_id, stage_id, title, value, probability${deal.won_at ? ', won_at' : ''}${deal.lost_at ? ', lost_at, lost_reason' : ''})
