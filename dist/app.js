...
import { createHttpHandler, Server, bodyParserMiddleware, corsMiddleware, sessionMiddleware, errorHandlerMiddleware, } from '@gao/http';
...
async function main() {
    ...
    const handleError = (error, statusCode, req, res, debug) => {
        const acceptsHtml = (req.header?.('accept') ?? '').includes('text/html');
        ...
        const errCode = error?.code ?? 'INTERNAL_SERVER_ERROR';
    ...
    const compositeHandler = async (req, res) => {
