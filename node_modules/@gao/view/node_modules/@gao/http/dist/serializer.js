 ...
 */
export function Serialize(dtoClass) {
    return (targetOrValue, propertyKeyOrContext, descriptor) => {
        if (!propertyKeyOrContext) {
            ...
            Reflect.defineMetadata(SERIALIZE_DECORATOR_KEY, dtoClass, targetOrValue, propertyKeyOrContext);
            return descriptor;
        }
        else {
            // Stage 3 method decorator
            Reflect.defineMetadata(SERIALIZE_DECORATOR_KEY, dtoClass, targetOrValue, propertyKeyOrContext.name);
 ...
 */
export function sanitizePayload(payload, DtoClass) {
    ...
    const allowedKeys = Object.keys(instance).length > 0 ? Object.keys(instance) : Object.keys(payload);
