...
export class GaoViewEngine {
    ...
    preprocessDirectives(template) {
        ...
        // @empty(array)
        result = replaceDirective(result, 'empty', (c) => `<@ if (Array.isArray(${c}) && (${c}).length === 0) { @>`);
        ...
        // @json(data)
        result = replaceDirective(result, 'json', (c) => `<@ __target(__escape(JSON.stringify(${c}))); @>`);
        // @class({ 'active': condition, 'disabled': otherCondition })
        result = replaceDirective(result, 'class', (c) => `<@ __target(Object.entries(${c}).filter(([,v]) => v).map(([k]) => k).join(' ')); @>`);
        // ─── @gao/ui directives (active when gaoUIPlugin is registered) ───
        // @fonts(['GaoSans', 'GaoMono']) → injects font CSS
        result = replaceDirective(result, 'fonts', (c) => `<@ if (typeof injectFonts === 'function') __target(injectFonts(${c})); @>`);
        // @icon('home', { size: 20 }) → renders SVG icon
        result = replaceDirective(result, 'icon', (c) => `<@ if (typeof gaoIcon === 'function') __target(gaoIcon(${c})); @>`);
        // @adminLayout({ sidebar: [...] }) → full admin page layout
        result = replaceDirective(result, 'adminLayout', (c) => `<@ if (typeof adminLayout === 'function') __target(adminLayout(${c})); @>`);
        return result;
    }
    ...
    async renderInternal(viewName, data) {
        const execute = (name, d) => {
            ...
            if (!fn || !this.options.cache) {
                ...
                if (this.options.cache)
                    this.fileCache.set(filePath, fn);
            }
    ...
    createHelpers(execute, data) {
        let currentSectionName = null;
        let sectionBuffer = '';
        let currentComponentName = null;
        let currentComponentProps = null;
        let componentBuffer = '';
        return {
            __escape: this.escape.bind(this),
            layout: (name) => { this.currentLayout = name; return ''; },
            section: (name) => {
            ...
            partial: (name, pData) => execute(name, { ...data, ...pData }),
            yieldSection: (name) => this.sections[name] || '',
            ...
            endcomponent: () => {
                ...
                if (comp.render)
                    return comp.render(props, slots);
                if (comp.prototype && comp.prototype.render)
                    return new comp().render(props, slots);
                return '';
            },
            __write: (text) => { if (currentSectionName)
                sectionBuffer += text; },
            __writeComponent: (text) => { if (currentComponentName)
                componentBuffer += text; },
            // Inject global helpers
            url: (path) => this.helpers.url(path),
            asset: (path) => this.helpers.asset(path),
            csrf: () => this.helpers.csrf(),
            can: (p) => this.helpers.can(p),
            old: (key, def) => this.helpers.old(key, data, def),
            paginate: (meta) => this.helpers.paginate(meta),
            // Spread UI helpers from @gao/ui plugin (if registered)
            ...this.uiHelpers,
        };
    }
    resolvePath(viewName) {
