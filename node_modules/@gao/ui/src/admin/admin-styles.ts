...
// ─── CRM App-Specific Classes (bridging inline styles ↔ design tokens) ─────

const CRM_CLASSES = `
/* ═══════════════════════════════════════════════════════════════
 * GAO CRM — Application-Level Component Classes
 * These ensure all controller-generated HTML respects the theme.
 * ═══════════════════════════════════════════════════════════════ */

/* ── Card (alias for gao-admin-card) ── */
.gao-card {
  background: var(--gao-surface);
  border: 1px solid var(--gao-border-light);
  border-radius: var(--gao-radius-lg);
...
/* ── Ensure all modal cards are SOLID, not transparent ── */
[id$="Modal"] .gao-card,
[id$="Modal"] > .gao-card {
  background: var(--gao-surface) !important;
  border: 1px solid var(--gao-border) !important;
  box-shadow: var(--gao-shadow-xl) !important;
}

/* ── Global Text Color Overrides for Controllers ── */
.gao-admin-content {
  color: var(--gao-text);
}

.gao-admin-content h1,
.gao-admin-content h2,
.gao-admin-content h3,
.gao-admin-content h4 {
  color: var(--gao-text) !important;
}
...
.gao-admin-content label {
  color: var(--gao-text) !important;
}

/* ── Table Overrides for Controller Tables ── */
.gao-admin-content .gao-admin-table td {
  color: var(--gao-text);
}

.gao-admin-content .gao-admin-table th {
  color: var(--gao-text-secondary);
}

.gao-admin-content .gao-admin-table a {
  color: var(--gao-primary) !important;
}

.gao-admin-content .gao-admin-table tr:hover td {
  background: var(--gao-gray-50);
}

/* ── Fix inline style color problems ── */
/* Override hardcoded dark-mode colors for light mode */
/* IMPORTANT: Exclude dropdown menus to preserve their intended colors */
.gao-admin-content [style*="color:#e2e8f0"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *),
.gao-admin-content [style*="color:#cbd5e1"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *),
.gao-admin-content [style*="color: #e2e8f0"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *),
.gao-admin-content [style*="color: #cbd5e1"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *) {
  color: var(--gao-text) !important;
}

.gao-admin-content [style*="color:#94a3b8"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *),
.gao-admin-content [style*="color: #94a3b8"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *),
.gao-admin-content [style*="color:#64748b"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *),
.gao-admin-content [style*="color: #64748b"]:not(.gao-crm-dropdown-item):not(.gao-crm-dropdown-menu *) {
  color: var(--gao-text-secondary) !important;
}

/* ── Input & Form Field Fix ── */
.gao-admin-content input[type="text"],
.gao-admin-content input[type="email"],
.gao-admin-content input[type="url"],
.gao-admin-content input[type="tel"],
.gao-admin-content input[type="number"],
.gao-admin-content input[type="password"],
.gao-admin-content input[type="date"],
.gao-admin-content input[type="datetime-local"],
.gao-admin-content select,
.gao-admin-content textarea {
  background: var(--gao-surface-raised) !important;
  border: 1px solid var(--gao-border) !important;
  border-radius: var(--gao-radius-md) !important;
  color: var(--gao-text) !important;
  font-family: var(--gao-font-sans) !important;
  padding: 10px 14px !important;
  font-size: 14px !important;
  outline: none;
  transition: border-color var(--gao-transition-fast), box-shadow var(--gao-transition-fast);
}

.gao-admin-content input:focus,
.gao-admin-content select:focus,
.gao-admin-content textarea:focus {
  border-color: var(--gao-primary) !important;
  box-shadow: 0 0 0 3px var(--gao-primary-bg) !important;
}

.gao-admin-content input::placeholder,
.gao-admin-content textarea::placeholder {
  color: var(--gao-text-muted) !important;
}
...
/* ── Card inner elements readability ── */
.gao-card h1, .gao-card h2, .gao-card h3, .gao-card h4 {
  color: var(--gao-text);
}

.gao-card [style*="font-weight:600"],
.gao-card [style*="font-weight:700"],
.gao-card [style*="font-weight:800"] {
  color: var(--gao-text);
}

/* ── Stat cards ── */
.gao-card[style*="text-align:center"] div:last-child {
  color: var(--gao-text-secondary) !important;
}

/* ── Grid detail info labels ── */
.gao-admin-content [style*="display:grid"] > div[style*="font-weight:600"] {
  color: var(--gao-text-secondary) !important;
}

/* ── Back links ── */
.gao-admin-content a[href][style*="color:#94a3b8"],
.gao-admin-content a[style*="color:#94a3b8"] {
  color: var(--gao-text-muted) !important;
}

.gao-admin-content a[href][style*="color:#94a3b8"]:hover {
  color: var(--gao-primary) !important;
}

/* ── Settings page cards ── */
.gao-admin-content a[href^="/settings/"] .gao-card,
.gao-admin-content a[href^="/settings/"] > div {
  transition: transform var(--gao-transition-fast), box-shadow var(--gao-transition-fast);
}

.gao-admin-content a[href^="/settings/"] .gao-card:hover,
.gao-admin-content a[href^="/settings/"]:hover > div {
  transform: translateY(-3px);
  box-shadow: var(--gao-shadow-lg);
}

/* ── Badges & status pills — keep their color ── */
.gao-admin-content span[style*="border-radius:8px"][style*="font-weight:700"],
.gao-admin-content span[style*="border-radius:10px"][style*="font-weight:700"] {
  /* Allow badge colors to pass through */
}

/* ── Cancel/secondary buttons ── */
.gao-admin-content a[style*="background:rgba(255,255,255,0.05)"] {
  background: var(--gao-gray-100) !important;
  color: var(--gao-text-secondary) !important;
  border: 1px solid var(--gao-border) !important;
}

.gao-admin-content a[style*="background:rgba(255,255,255,0.05)"]:hover {
  background: var(--gao-gray-200) !important;
  color: var(--gao-text) !important;
}

/* ── Fix opacity/bg for inline bg elements ── */
.gao-admin-content [style*="background:rgba(15,23,42"] {
  background: var(--gao-surface-raised) !important;
}

.gao-admin-content [style*="background:rgba(255,255,255,0.03)"],
.gao-admin-content [style*="background:rgba(255,255,255,0.02)"] {
  background: var(--gao-gray-50) !important;
}

/* ── Ticket messages ── */
.gao-admin-content [style*="background:rgba(15,23,42,0.4)"] {
  background: var(--gao-gray-50) !important;
  border: 1px solid var(--gao-border-light) !important;
}

.gao-admin-content [style*="background:rgba(245,158,11,0.08)"] {
  background: var(--gao-warning-bg) !important;
  border: 1px solid hsla(38, 92%, 50%, 0.15) !important;
}

/* ── Border colors from inline styles ── */
.gao-admin-content [style*="border-top:1px solid rgba(100,116,139"],
.gao-admin-content [style*="border-bottom:1px solid rgba(100,116,139"] {
  border-color: var(--gao-border-light) !important;
}

/* ── Category/filter pills ── */
.gao-admin-content [style*="background:rgba(99,102,241,0.08)"] {
  background: var(--gao-primary-bg) !important;
  border: 1px solid hsla(var(--gao-primary-h), var(--gao-primary-s), var(--gao-primary-l), 0.15) !important;
}

/* ── Delete button ── */
.gao-admin-content button[style*="background:rgba(239,68,68,0.1)"],
.gao-admin-content [style*="background:rgba(239,68,68,0.1)"] {
  background: var(--gao-danger-bg) !important;
  color: var(--gao-danger) !important;
  border: 1px solid hsla(350, 72%, 52%, 0.2) !important;
}

/* ── Status change button ── */
.gao-admin-content button[style*="background:rgba(99,102,241,0.15)"] {
  background: var(--gao-primary-bg) !important;
  color: var(--gao-primary) !important;
  border: 1px solid hsla(var(--gao-primary-h), var(--gao-primary-s), var(--gao-primary-l), 0.2) !important;
}

/* ── Scrollbar styling for dark mode ── */
@media (prefers-color-scheme: dark) {
  :root:not([data-theme="light"]) .gao-admin-content::-webkit-scrollbar {
    width: 6px;
  }
  :root:not([data-theme="light"]) .gao-admin-content::-webkit-scrollbar-track {
    background: transparent;
  }
  :root:not([data-theme="light"]) .gao-admin-content::-webkit-scrollbar-thumb {
    background: var(--gao-gray-600);
    border-radius: 3px;
  }
}

/* ── CRM Dropdown Menu (Quick Add, etc.) ── */
.gao-crm-dropdown-menu {
  background: var(--gao-surface) !important;
  border: 1px solid var(--gao-border) !important;
  box-shadow: var(--gao-shadow-xl) !important;
}

.gao-crm-dropdown-item {
  color: var(--gao-text) !important;
}

.gao-crm-dropdown-item:hover {
  background: var(--gao-primary-bg) !important;
}

/* ── Micro-Animations (Sprint 6) ── */
@keyframes gao-crm-fadeInUp {
  from { opacity: 0; transform: translateY(12px); }
  to   { opacity: 1; transform: translateY(0); }
}

/* Staggered stat card entrance */
.gao-admin-content .gao-admin-stat-card:nth-child(1),
.gao-admin-content > div > div > a:nth-child(1) > .gao-admin-stat-card { animation: gao-crm-fadeInUp 0.3s ease-out 0s both; }
.gao-admin-content .gao-admin-stat-card:nth-child(2),
.gao-admin-content > div > div > a:nth-child(2) > .gao-admin-stat-card { animation: gao-crm-fadeInUp 0.3s ease-out 0.06s both; }
.gao-admin-content .gao-admin-stat-card:nth-child(3),
.gao-admin-content > div > div > a:nth-child(3) > .gao-admin-stat-card { animation: gao-crm-fadeInUp 0.3s ease-out 0.12s both; }
.gao-admin-content .gao-admin-stat-card:nth-child(4),
.gao-admin-content > div > div > a:nth-child(4) > .gao-admin-stat-card { animation: gao-crm-fadeInUp 0.3s ease-out 0.18s both; }
.gao-admin-content .gao-admin-stat-card:nth-child(5),
.gao-admin-content > div > div > a:nth-child(5) > .gao-admin-stat-card { animation: gao-crm-fadeInUp 0.3s ease-out 0.24s both; }
.gao-admin-content .gao-admin-stat-card:nth-child(6),
.gao-admin-content > div > div > a:nth-child(6) > .gao-admin-stat-card { animation: gao-crm-fadeInUp 0.3s ease-out 0.3s both; }

/* Table row hover accent bar */
.gao-admin-table tr {
  border-left: 3px solid transparent;
  transition: background var(--gao-transition-fast), border-color var(--gao-transition-fast);
}
.gao-admin-table tr:hover {
  border-left-color: var(--gao-primary);
}

/* Page content entrance */
.gao-admin-content > div:first-child {
  animation: gao-crm-fadeInUp 0.35s ease-out;
}
