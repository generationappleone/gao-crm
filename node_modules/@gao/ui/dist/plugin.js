...
import { statCard, dataTable, barChart, lineChart, donutChart, form, breadcrumb, toast, modal, badge, progress, avatar, emptyState, alertBanner, } from './admin/admin-components.js';
// ─── Plugin UI Helpers ───────────────────────────────────────
/**
 ...
 */
export function createUIHelpers(options = {}) {
    const { fonts = true, icons = true, admin = true, defaultFonts } = options;
    const helpers = {};
    if (fonts) {
        helpers['injectFonts'] = (names) => injectFonts(names ?? defaultFonts);
        helpers['fontCSS'] = (names) => generateFontCSS(names ?? defaultFonts);
    }
    if (icons) {
        helpers['gaoIcon'] = (name, opts) => gaoIcon(name, opts);
        helpers['gaoIconSprite'] = (names) => gaoIconSprite(names);
 ...
 * ```
 */
export function gaoUIPlugin(options = {}) {
    return {
        name: '@gao/ui',
        version: '0.1.0',
        /**
