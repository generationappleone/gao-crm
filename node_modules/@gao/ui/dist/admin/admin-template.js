...
export const createAdminTemplate = {
     ...
     */
    layout(config) {
        const { title, brandName = 'GaoAdmin', brandIcon = 'layout', sidebar: sidebarItems, navbar = {}, content, footer, } = config;
        ...
        const fontsHtml = injectFonts(config.fonts);
...
<script>${adminScriptsInline()}</script>
     ...
     */
    sidebar(items, brandName = 'GaoAdmin', brandIcon = 'layout') {
        let currentSection = '';
        const navItems = [];
        for (const item of items) {
            ...
            const badge = item.badge
                ...
                : '';
            navItems.push(`<a href="${escapeHtml(href)}" class="gao-admin-sidebar-item${activeClass}">${icon}<span>${escapeHtml(item.label)}</span>${badge}</a>`);
        }
...
//# sourceMappingURL=admin-template.js.map