...
export class BudgetExceededError extends RateLimitError {
    dimension;
    constructor(dimension, message, retryAfter = 60) {
 ...
 */
export function runWithBudget(context, fn) {
    return budgetStorage.run(context, fn);
}
/**
 * Create a new budget context for a request.
 */
export function createBudgetContext(requestId, plan) {
    return {
        requestId,
        plan,
        startTime: Date.now(),
...
export function trackDbQuery() {
    ...
    if (ctx.dbQueryCount > ctx.plan.maxDbQueries) {
        throw new BudgetExceededError('DB_QUERIES', `DB query budget exceeded: ${ctx.dbQueryCount}/${ctx.plan.maxDbQueries} queries`);
    }
}
/**
 ...
 * No-op if called outside a budget scope.
 */
export function trackResponseBytes(bytes) {
    const ctx = budgetStorage.getStore();
    if (!ctx)
        return;
    ctx.responseBytes += bytes;
    if (ctx.responseBytes > ctx.plan.maxResponseBytes) {
        throw new BudgetExceededError('RESPONSE_SIZE', `Response size budget exceeded: ${ctx.responseBytes}/${ctx.plan.maxResponseBytes} bytes`);
    }
}
/**
 ...
 * No-op if called outside a budget scope.
 */
export function checkCpuBudget() {
    const ctx = budgetStorage.getStore();
    if (!ctx)
        return;
    const elapsed = Date.now() - ctx.startTime;
    if (elapsed > ctx.plan.maxCpuTimeMs) {
        throw new BudgetExceededError('CPU_TIME', `CPU time budget exceeded: ${elapsed}ms/${ctx.plan.maxCpuTimeMs}ms`);
    }
}
