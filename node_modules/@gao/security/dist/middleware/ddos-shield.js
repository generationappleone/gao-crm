 ...
 * DDoS Shield
 */
export function ddosShield(options = {}) {
    ...
    return {
        ...
        handle: async (ctx, next) => {
            ...
            if (state.concurrent >= maxConcurrent) {
                throw new RateLimitError(`Too many concurrent connections from your IP (max: ${maxConcurrent})`, 1);
            }
            ...
            if (state.burstCount > burstLimit) {
                ...
                throw new RateLimitError(`Flood detected. IP banned for ${options.banDurationSeconds ?? 300} seconds.`, options.banDurationSeconds ?? 300);
            }
...
function defaultGetIp(ctx) {
    const request = ctx.metadata.request;
    return request?.ip ?? 'unknown';
}
//# sourceMappingURL=ddos-shield.js.map